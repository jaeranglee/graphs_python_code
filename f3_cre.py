...
ax_left.plot(df_merged['observation_date'], df_merged['CREACBW027SBOG']/1e3, label='CRE대출(좌축)', color='red', lw=2)
ax_right.plot(df_merged['observation_date'], df_merged['ratio_cre_small'], label='소규모은행 비중(우축)', color='orange', lw=2)
...
ax_right1.plot(df_merged['observation_date'], df_merged['ratio_home_small'], label='소규모은행 비중(우축)', color='green', lw=2)
...
# Plotting
ax[2].plot(df_price['observation_date'], df_price['COMREPUSQ159N'], label='CRE 가격', color='red', lw=2)
