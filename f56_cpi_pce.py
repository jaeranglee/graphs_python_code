...
ax[0].plot(pce_df['observation_date'], pce_df['YoY_pce'], color='r', lw=2, label='PCE 물가지수')


# Core CPI, Core PCE(오른쪽)
ax[1].plot(df_corens['observation_date'], df_corens['YoY_Inflation'], lw=2, color='blue', label='근원소비자 물가지수')
ax[1].plot(pce_core_df['observation_date'], pce_core_df['YoY_pcecore'], lw=2, color='red', label='근원PCE 물가지수')
...
for i in [0,1]:
    for start_date, end_date in shaded_periods:
        ...
        # 단위 표시 및 출처
        ax[i].text(0, 1.00, '(%, 전년동월대비)', ha='left', va='bottom', color='blue',
