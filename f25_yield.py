...
for i in [0,1]:
    if i == 0:
        ...
        ax.plot(old_curve.index, old_curve.values, color='red', ls="--", marker='o', lw=1, label=str(old_dt))
        ax.plot( july_curve.index, july_curve.values, color='blue', marker='o', lw=2, label=str(july_dt))

        # 코드를 실행하는 날에 가장 근접한 yield curve
        # ax.plot(latest_curve.index, latest_curve.values, color='red', marker='o', lw=2, label=str(latest_date.date()))

        # 책에 사용된 yield curve
        ax.plot(book_curve.index, book_curve.values, color='red', marker='o', lw=2, label=str(book_dt))
